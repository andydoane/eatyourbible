(async function(){
  ...
  function handleBottomMiss(target){
    ...
    addEffect({ kind:"beam", x:target.x, y:state.bottomZoneY - 58, until:performance.now() + 860 });
    renderHud();
    renderDynamic();
  ...
  function renderDynamic(){
    ...
    effectsEl.innerHTML = state.effects.map((effect) => {
      ...
      if (effect.kind === "burst") return `<div class="vinv-burst ${effect.large ? "is-large" : ""}" style="left:${effect.x}px; top:${effect.y}px;">${effect.icon}</div>`;
