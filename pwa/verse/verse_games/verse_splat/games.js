...
function gameplayShell({ bonus=false }){
  ...
  function effectNodeAt(x, y, markup, layerId="vspEffectLayer"){
  ...
  function backEffectNodeAt(x, y, markup){
    return effectNodeAt(x, y, markup, "vspBackEffectLayer");
  }
  ...
  function spawnSplatEffect(blob, center=null){
    const hitCenter = center || blobCenterPx(blob);
    ...
    const node = backEffectNodeAt(hitCenter.x, hitCenter.y, markup);
    if (node) setTimeout(() => node.remove(), 620);
  }

  function particleBurstMarkup(fill){
    ...
    for (let i = 0; i < count; i++){
      ...
      const stretch = 0.55 + Math.random() * 0.75;
      const rot = (-45 + Math.random() * 90).toFixed(2);
      const life = 380 + Math.random() * 180;
      pieces += `<span class="vsp-particle" style="--p-color:${fill};--p-size:${size.toFixed(1)}px;--p-stretch:${stretch.toFixed(3)};--p-rot:${rot}deg;--p-x:${dx.toFixed(1)}px;--p-y:${dy.toFixed(1)}px;--p-life:${life.toFixed(0)}ms;"></span>`;
    }
    return `<div class="vsp-particle-burst">${pieces}</div>`;
  }

  function spawnParticleBurst(blob, center=null){
    const hitCenter = center || blobCenterPx(blob);
    const node = backEffectNodeAt(hitCenter.x, hitCenter.y, particleBurstMarkup(blob.color));
    if (node) setTimeout(() => node.remove(), 760);
  }

  function playCorrectBurstEffects(blob, center=null){
    spawnSplatEffect(blob, center);
    spawnParticleBurst(blob, center);
  }
  ...
  function handleBonusBlobTap(id){
    ...
    playCorrectBurstEffects(blob, center);
    blob.alive = false;
    state.bonusScore += 1;
